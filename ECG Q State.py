...
if file_path:
    ...
    start_sample = 2041 * 1000        # = 0
    end_sample   = 2280 * 1000     # = 300,000
    ...
    plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: Unfiltered Raw Signal from variable: {var_name}')
...
plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: Power Spectral Density - Raw Unfiltered ECG Signal')
plt.xlim(0, 100)
...
plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: Filtered Signal: Powerline Noise & Baseline Centered')
plt.legend()
plt.xlabel("Samples [N]")
...
plt.plot(quality)
plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: Quality Check')
plt.xlabel("Samples [N]")
...
plt.xlabel("Samples [N]")
plt.ylabel("Correlation Coefficient [r]")
plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: Detected High-Quality Segments (r > 0.95)')
plt.show()
...
plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: R-Peak Validation')
plt.xlim(4000, 5000)
plt.xlabel("Samples [N]")
plt.ylabel("Voltage [mV]")
plt.show()
...
plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: RR Intervals from R-peaks')
...
plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: Cleaned RR Intervals (Correct Series)')
plt.xlabel("Beat number")
...
plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: Filtered ECG - Last 10 R-peaks')
plt.xlabel("Samples [N]")
plt.ylabel("Amplitude [mV]")
plt.legend(loc='upper left')
plt.grid(True)
...
def compute_edr(ecg_filtered, peaks_cleaned, sampling_rate=1000):
    ...
    plt.suptitle(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: ECG-Derived Respiration (EDR)', fontsize=12, fontweight='bold')
    plt.tight_layout()
    plt.show()
...
def compute_delineation(ecg_filtered, peaks_cleaned, sampling_rate=1000):
    ...
    plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: ECG Delineation - P, Q, R, S, T Waves')
...
def compute_ecg_rate(ecg_filtered, peaks_cleaned, sampling_rate=1000):
    ...
    plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: ECG Heart Rate Over Time')
    plt.xlabel("Samples [N]")
...
plt.title(f'{TITLE_PREFIX}: "Q" Quiet Sleep State: Original RR Intervals vs Interpolated Heart Rate')
plt.grid(True)
plt.tight_layout()
plt.show()
